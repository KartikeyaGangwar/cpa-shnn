...
def run_system3_benchmark(device=None):
    ...
    print("\n" + "#"*70)
    print("  EXECUTING SYSTEM III BENCHMARK: ELLIPTIC SITNIKOV FIVE-BODY PROBLEM")
    print("#"*70)
    
    system = EllipticSitnikovFiveBodySystem(device=device)
    ...
    results = {
        "system": "EllipticSitnikovFiveBodySystem",
        "standard_pinn": {
    ...
    with open("results/data/system3_sitnikov_results.json", "w") as f:
        json.dump(results, f, indent=4)
        
    print(f"\n[+] System III Benchmark Complete! AS-PINN Rel L2: {as_res['final_rel_l2_err']*100:.3f}% vs Std PINN: {std_res['final_rel_l2_err']*100:.3f}%")
    return results
