...
def run_system4_benchmark(device=None):
    ...
    print("\n" + "#"*70)
    print("  EXECUTING SYSTEM IV BENCHMARK: PHOTOGRAVITATIONAL MAGNETIC YUKAWA BINARY")
    print("#"*70)
    
    system = PhotogravitationalMagneticYukawaBinary(device=device)
    ...
    results = {
        "system": "PhotogravitationalMagneticYukawaBinary",
        "standard_pinn": {
    ...
    with open("results/data/system4_yukawa_results.json", "w") as f:
        json.dump(results, f, indent=4)
        
    print(f"\n[+] System IV Benchmark Complete! AS-PINN Rel L2: {as_res['final_rel_l2_err']*100:.3f}% vs Std PINN: {std_res['final_rel_l2_err']*100:.3f}%")
    return results
