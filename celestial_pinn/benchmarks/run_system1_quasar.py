...
def run_system1_benchmark(device=None):
    device = device or torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("\n" + "#"*70)
    print("  EXECUTING SYSTEM I BENCHMARK: BINARY QUASAR MODEL")
    print("#"*70)
    
    system = BinaryQuasarSystem(device=device)
    ...
    with open("results/data/system1_quasar_results.json", "w") as f:
        json.dump(results, f, indent=4)
        
    print(f"\n[+] System I Benchmark Complete! AS-PINN Rel L2: {as_res['final_rel_l2_err']*100:.3f}% vs Std PINN: {std_res['final_rel_l2_err']*100:.3f}%")
    return results

if __name__ == "__main__":
    run_system1_benchmark()
